...
Hausarbeit im Kurs *Advanced Data Analysis with R* (ADA).
...
¹ *New submission* (jedes noch nicht auf CRAN veröffentlichte Paket), *unable
to verify current time* (Prüfrechner ohne Zeitserver-Zugriff) und *Skipping
checking HTML validation* (kein aktuelles HTML Tidy installiert). Keine der
